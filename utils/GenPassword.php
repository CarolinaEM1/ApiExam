 ...
 * - Usa Fisher–Yates para mezclar de forma segura.
 */
...
function generate_password(int $length = 16, array $opts = []): string {
    ...
    if ($pool === '') {
        throw new InvalidArgumentException("No hay caracteres disponibles para generar la contraseña.");
    }

    $password_chars = [];

    // Garantizar al menos uno de cada categoría seleccionada
    ...
    return shuffle_secure($password);
}
...
function generate_passwords(int $count = 5, int $length = 16, array $opts = []): array {
    ...
    if ($count < 1 || $count > 50) {
        throw new InvalidArgumentException("El número de contraseñas debe estar entre 1 y 50.");
    }
